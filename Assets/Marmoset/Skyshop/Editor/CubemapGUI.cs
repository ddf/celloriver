...
namespace mset {
	public class CubemapGUI {
		...
		private void drawButtons( int buttonHeight ){ 			
			...
			string beastTip = "Export image and set the Beast Lightmapper to use it as global illumination\n\nRequires Beast settings file, see \"Beast Global Illum Options\" for more details";
			...
			EditorGUILayout.BeginHorizontal();{
				...
				if( bNewProbe ) {
					...
					if( GUI.Button(probeRect,content) ) {
						...
						string[] names = new string[] {"64", "128", "256", "512", "1024"};
				...
				if( bAsGI ) {
					...
					Rect giRect = GUILayoutUtility.GetRect(content, buttonStyle, GUILayout.Width(75), GUILayout.Height(buttonHeight));					
		...
		private void renderProbeCallback(object data) {
			...
			probe.capture(Selection.activeTransform);
			//not supported yet :-(
			this.HDR = false;
