	...
	public class Util
	{
		...
		*/
		...
		public static void invCubeLookup( ref Vector3 dst, ref float weight, ulong face, ulong col, ulong row, ulong faceSize ) {
			float uvshift = 0.5f;
			float invFaceSize = 1/(float)faceSize;
			float x = 2 * ((float)col+uvshift) * invFaceSize - 1f;
			float y = 2 * ((float)row+uvshift) * invFaceSize - 1f;
			switch( face ) {
			...
			float mag2 = dst.sqrMagnitude;
			weight = 1f / ( mag2 * Mathf.Sqrt(mag2) ); 
			dst.Normalize();
		}
